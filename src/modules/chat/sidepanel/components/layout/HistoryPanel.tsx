    ...
    return (
        <div className="absolute inset-0 z-50 flex flex-col bg-[var(--chrome-bg)] text-[var(--chrome-text)] animate-in slide-in-from-left duration-200">
            ...
            <div className="px-4 pb-4">
                <div className="bg-[var(--chrome-input-bg)] rounded-xl p-3 flex items-center justify-between group cursor-pointer hover:opacity-80 transition-opacity">
                    <div className="flex items-center gap-2">
                        <MonitorSmartphone className="w-4 h-4 opacity-70" />
                        <span className="text-xs font-medium opacity-90">Sync chats across devices</span>
                    </div>
                </div>
            </div>

            {/* Tabs */}
            <div className="px-4 flex gap-6 border-b border-[var(--chrome-border)] mb-2">
                <button
                    onClick={() => setActiveTab('all')}
                    className={`pb-2 text-sm font-medium transition-colors relative ${activeTab === 'all' ? 'opacity-100' : 'opacity-40 hover:opacity-70'
                        }`}
                >
                    All
                    {activeTab === 'all' && (
                        <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-[var(--chrome-text)] opacity-60 rounded-t-full" />
                    )}
                </button>
                <button
                    onClick={() => setActiveTab('starred')}
                    className={`pb-2 text-sm font-medium transition-colors relative ${activeTab === 'starred' ? 'opacity-100' : 'opacity-40 hover:opacity-70'
                        }`}
                >
                    Starred
                    {activeTab === 'starred' && (
                                ...
                                {items.map(convo => (
                                    ...
                                    >
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="text-xs opacity-40 truncate line-clamp-1">
            ...
            <div className="p-4 border-t border-[var(--chrome-border)] text-center">
                <button className="text-xs font-medium flex items-center justify-center gap-1 w-full py-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg transition-colors opacity-60 hover:opacity-100">
                    Upgrade Plan
                </button>
            </div>
        </div>
